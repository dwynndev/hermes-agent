...
describe('useModelControls', () => {
  ...
  it('keeps a mid-turn pick painted and skips the refetch that would repaint the old model', async () => {
    // The backend parks a switch made during a turn (scope="pending") and
    // applies it when the turn ends. Invalidating now would answer with the
    // still-running model and overwrite the user's choice in the pill.
    $activeSessionId.set('session-1')
    const requestGateway = vi.fn(async () => ({ key: 'model', scope: 'pending', value: 'grok-4.5' }) as never)
